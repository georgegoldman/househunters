...
import FavouriteIcon from "../assets/favourite-icon";
import RequestReviewModal from "./request-review-modal"; // Keep your existing modal
import { toPropertyCard, type ApiProperty, type PropertyCardItem } from "../lib/mappers";
...
async function validateImage(url: string, timeoutMs = 6000): Promise<string | null> {
...
function toViewingRequestPayload(formValues: any): CreatePropertyRequestPayload {
  // Handle different form field names that your existing modal might use
  const firstName = formValues.firstName || formValues.first_name || formValues.name?.split(' ')?.[0] || '';
  const lastName = formValues.lastName || formValues.last_name || formValues.name?.split(' ')?.slice(1)?.join(' ') || '';
  ...
  if (!firstName && !lastName && formValues.fullName) {
    ...
    return {
      ...
      phoneNumber: formValues.phoneNumber || formValues.phone || formValues.phoneNo || '',
      preferredDate: formValues.preferredDate || formValues.date || formValues.viewingDate || '',
      preferredTime: formValues.preferredTime || formValues.time || formValues.viewingTime || '',
      additionalInfo: formValues.additionalInfo || formValues.message || formValues.notes || formValues.comment || '',
      specialRequirements: formValues.specialRequirements || formValues.requirements || '',
    };
  }
...
const PropertyPreview = () => {
  ...
  // Raw list from API
  const rawImages = useMemo(() => (item?.images ?? []).filter(Boolean), [item?.images]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const results = await Promise.all(normalizedImages.map((u) => validateImage(u)));
  ...
  const prevLightbox = () =>
    setLightboxIndex((i) => (i - 1 + Math.max(1, validatedImages.length)) % Math.max(1, validatedImages.length));
  ...
  const handleCreateViewingRequest = async (formValues: any) => {
    ...
    const payload: CreatePropertyRequestPayload = toViewingRequestPayload(formValues);
    ...
    try {
      ...
      const result = await api.propertyRequests.create(Number(item.id), payload);
  ...
  return (
    <div>
      <header className="w-full h-[65vh] bg-black/70 relative">
        <Navbar />
      ...
      <main className="flex flex-col lg:flex-row items-start gap-[clamp(1rem,4vw,2rem)] main-container py-[clamp(2rem,5vw,3rem)]">
        ...
        <section className="w-full lg:w-[clamp(25rem,45vw,41.75rem)] relative">
          ...
          >
            <img
              ...
              alt={`${id ?? "property"} view ${validatedImages.length ? currentImageIndex + 1 : 1}`}
            />
          ...
          <div
            className={`
              ...
              ${item?.status === "Sold" ? "bg-red-500 text-white" : "bg-white text-gray-900"}
          ...
          {isPopular && (
            ...
            >
              <StarIcon />
              <p className="font-bold text-[clamp(0.625rem,1.5vw,0.75rem)]">POPULAR</p>
            </div>
          )}
        </section>
        ...
        <section className="flex flex-col justify-between gap-[clamp(2rem,5vw,2.5rem)] p-[clamp(1rem,3vw,1.25rem)] flex-1">
          <div className="flex flex-col gap-[clamp(1.5rem,4vw,1.5rem)]">
            {/* Price */}
            <p className="font-bold text-[clamp(1.5rem,4vw,2.5rem)]">{item?.price}</p>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-[clamp(0.75rem,2vw,1rem)] pb-[clamp(0.5rem,1.5vw,0.75rem)] border-b border-black/10">
              ...
              <a
                ...
                aria-label={`Call agent ${item?.agent?.name ?? ""} at ${item?.agent?.phone ?? ""}`}
              >
                <CallIcon />
                <span className="text-[clamp(0.875rem,2vw,1rem)] text-black/90">
                  {item?.agent?.phone || "+1 234 567 8900"}
                </span>
              </a>
            </div>

            {/* Property Features */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-[clamp(0.75rem,3vw,1.25rem)]" role="list" aria-label="Property features and amenities">
              <div className="flex items-center gap-[clamp(0.25rem,1vw,0.5rem)]" role="listitem">
                <BedIcon />
                <p className="text-[clamp(0.875rem,2vw,1rem)]">{item?.bedrooms || 2} Bed</p>
              </div>
              <div className="flex items-center gap-[clamp(0.25rem,1vw,0.5rem)]" role="listitem">
                <BathtubIcon />
                <p className="text-[clamp(0.875rem,2vw,1rem)]">{item?.bathrooms || 2} Bath</p>
              </div>
              <div className="flex items-center gap-[clamp(0.25rem,1vw,0.5rem)]" role="listitem">
                <SofaIcon />
              ...
              <div className="flex items-center gap-[clamp(0.25rem,1vw,0.5rem)]" role="listitem">
                ...
                <p className="text-[clamp(0.875rem,2vw,1rem)]">
                  {item?.area ? `${item.area}m²` : item?.features?.[0]?.label ?? "—"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-[clamp(1rem,3vw,1.25rem)]">
            ...
            <div className="flex gap-[clamp(0.75rem,2vw,1.25rem)]">
              <button className="border border-black/20 rounded-[clamp(0.5rem,1vw,0.75rem)] flex justify-center items-center gap-[clamp(0.5rem,1vw,0.75rem)] py-[clamp(0.5rem,1.5vw,0.75rem)] px-[clamp(0.75rem,2vw,1rem)] flex-1 hover:bg-gray-50">
                <p className="text-black/70 text-[clamp(0.75rem,1.5vw,0.875rem)]">Share</p>
                <ShareIcon />
              </button>

              <button className="border border-black/20 rounded-[clamp(0.5rem,1vw,0.75rem)] flex justify-center items-center gap-[clamp(0.5rem,1vw,0.75rem)] py-[clamp(0.5rem,1.5vw,0.75rem)] px-[clamp(0.75rem,2vw,1rem)] flex-1 hover:bg-gray-50">
                <p className="text-black/70 text-[clamp(0.75rem,1.5vw,0.875rem)]">Like</p>
                <FavouriteIcon />
              </button>
            </div>
          </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-3 rounded-lg shadow-lg text-white ${
