...
// Full-card skeleton, same structure as real card but with blocks
...
export default function PropertyCard({ property }: PropertyProps) {
  ...
  return (
    <div className="relative">
      {/* Shimmer version – shown until main image loads */}
      {!cardLoaded && (
      ...
      {/* Real card – hidden until hero image finishes loading */}
      <article
        className={`
          pb-[clamp(0.75rem,2vw,1rem)] flex flex-col gap-[clamp(1rem,3vw,1.25rem)] 
          max-w-[clamp(20rem,40vw,25rem)] mx-auto
      ...
      >
        <div
          ...
          className="w-full h-[clamp(12rem,25vw,18.5rem)] relative cursor-pointer"
        >
          <img
            src={property.image}
            onLoad={() => setCardLoaded(true)}
            onError={() => setCardLoaded(true)} // still hide shimmer even if image fails
            className="w-full h-full rounded-[clamp(0.75rem,2vw,1.25rem)] object-cover"
